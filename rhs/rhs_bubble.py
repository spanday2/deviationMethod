...
def rhs_bubble(Q, geom, mtrx, nbsolpts, nb_elements_x, nb_elements_z):
   ...
   rho      = Q_total[idx_2d_rho,:,:]
   uu       = Q_total[idx_2d_rho_u,:,:] / rho
   ww       = Q_total[idx_2d_rho_w,:,:] / rho
   pressure = p0 * numpy.exp((cpd/cvd) * numpy.log((Rd/p0)*Q_total[idx_2d_rho_theta, :, :]))


   # --- Compute the fluxes
   flux_x1[idx_2d_rho,:,:]       = Q_total[idx_2d_rho_u,:,:]
   flux_x1[idx_2d_rho_u,:,:]     = Q_total[idx_2d_rho_u,:,:] * uu + pressure
   flux_x1[idx_2d_rho_w,:,:]     = Q_total[idx_2d_rho_u,:,:] * ww
   flux_x1[idx_2d_rho_theta,:,:] = Q_total[idx_2d_rho_theta,:,:] * uu

   flux_x3[idx_2d_rho,:,:]       = Q_total[idx_2d_rho_w,:,:]
   flux_x3[idx_2d_rho_u,:,:]     = Q_total[idx_2d_rho_w,:,:] * uu
   flux_x3[idx_2d_rho_w,:,:]     = Q_total[idx_2d_rho_w,:,:] * ww + pressure
   flux_x3[idx_2d_rho_theta,:,:] = Q_total[idx_2d_rho_theta,:,:] * ww
   ...
   for elem in range(nb_elements_z):
      ...
      kfaces_var[:,elem,0,:] = mtrx.extrap_down @ Q_total[:,epais,:]
      kfaces_var[:,elem,1,:] = mtrx.extrap_up @ Q_total[:,epais,:]

   for elem in range(nb_elements_x):
      epais = elem * nbsolpts + standard_slice

      ifaces_var[:,elem,:,0] = Q_total[:,:,epais] @ mtrx.extrap_west
      ifaces_var[:,elem,:,1] = Q_total[:,:,epais] @ mtrx.extrap_east
