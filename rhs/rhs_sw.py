...
def rhs_sw (Q: numpy.ndarray, geom, mtrx, metric, topo, ptopo, nbsolpts: int, nb_elements_hori: int):
   ...
   t_df1_dx1, t_df2_dx2, t_flux_x1, t_flux_x2 = [numpy.empty_like(Q, dtype=type_vec) for _ in range(4)]

   t_flux_x1_itf_i = numpy.empty((nb_equations, nb_elements_hori+2, nbsolpts*nb_elements_hori, 2), dtype=type_vec)
   t_flux_x2_itf_j, t_var_itf_i, t_var_itf_j= [numpy.empty((nb_equations, nb_elements_hori+2, 2, nbsolpts*nb_elements_hori), dtype=type_vec) for _ in range(3)]

   forcing = numpy.zeros_like(Q, dtype=type_vec)
   t_forcing = numpy.zeros_like(Q, dtype=type_vec)
